...
function toNumber(raw: string): number | null {
  ...
  if (hasComma && hasDot) {
    if (s.lastIndexOf(',') > s.lastIndexOf('.')) {
      s = s.replace(/\./g, '').replace(',', '.'); // EU: dot=thousands, comma=decimal
    } else {
      s = s.replace(/,/g, ''); // US/IN: comma=thousands
    }
  } else if (hasComma) {
    const parts = s.split(',');
    ...
    const parts = s.split('.');
    if (parts.length > 2) s = s.replace(/\./g, ''); // several dots => thousands
  }
  const n = Number(s);
  return Number.isFinite(n) ? n : null;
}

// A currency-ish token: at least one digit, optional separators & 0-2 decimals.
...
function numbersIn(line: string): number[] {
  ...
  for (const m of matches) {
    const n = toNumber(m);
    // Ignore years and obviously-not-money values (phone numbers, huge ids).
    if (n == null) continue;
