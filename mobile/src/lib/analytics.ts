...
export function insights(expenses: Expense[]): Insights {
  ...
  for (const [name, count] of catCount) {
    if (!frequentCategory || count > frequentCategory.count) {
      frequentCategory = { name, count };
    }
  }
...
// ── Period filtering (Day / Month / Year / All time) — mirrors the web ────────
...
export function periodLabel(
  ...
  if (view === 'month') return `${MONTH_LABELS[month]} ${year}`;
